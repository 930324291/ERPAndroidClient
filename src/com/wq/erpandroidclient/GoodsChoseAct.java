 ...
 *
 */
public class GoodsChoseAct extends Activity implements OnItemClickListener ,OnClickListener{
	ListView lv;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapter = new SuperAdapter(this,android.R.layout.simple_list_item_1,lvData) {
			...
			public void dataView(View convertView, ContentValues value, int position) {
				...
				tv.setText(value.getAsString("ItemName"));
	...
	private void search(String key){
		...
		for(ContentValues value : oData){
			if(value.getAsString("ItemName").contains(key)){
				lvData.add(value);
			}
		}
